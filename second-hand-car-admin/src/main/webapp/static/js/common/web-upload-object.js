	...
	$WebUpload.prototype = {
		...
		create : function() {
			var webUploader = WebUploader.create({
				...
				swf : SecondHandCarAdmin.ctxPath
		...
		bindEvent : function(bindedObj) {
			...
			bindedObj.on('uploadError', function(file) {
				SecondHandCarAdmin.error("上传失败");
			});

			// 其他错误
			bindedObj.on('error', function(type) {
				if ("Q_EXCEED_SIZE_LIMIT" == type) {
					SecondHandCarAdmin.error("文件大小超出了限制");
				} else if ("Q_TYPE_DENIED" == type) {
					SecondHandCarAdmin.error("文件类型不满足");
				} else if ("Q_EXCEED_NUM_LIMIT" == type) {
					SecondHandCarAdmin.error("上传数量超过限制");
				} else if ("F_DUPLICATE" == type) {
					SecondHandCarAdmin.error("图片选择重复");
				} else {
					SecondHandCarAdmin.error("上传过程中出错");
				}
			});
