...
OptLog.detail = function () {
    if (this.check()) {
        var ajax = new $ax(SecondHandCarAdmin.ctxPath + "/log/detail/" + this.seItem.id, function (data) {
            SecondHandCarAdmin.infoDetail("日志详情", data.regularMessage);
        }, function (data) {
            SecondHandCarAdmin.error("获取详情失败!");
        });
        ajax.start();
    }
};
...
OptLog.delLog = function () {
    SecondHandCarAdmin.confirm("是否清空所有日志?",function(){
        var ajax = SecondHandCarAdmin.baseAjax("/log/delLog","清空日志");
        ajax.start();
