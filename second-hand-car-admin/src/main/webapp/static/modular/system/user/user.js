...
MgrUser.delMgrUser = function () {
    if (this.check()) {
        ...
        SecondHandCarAdmin.confirm("是否删除用户" + MgrUser.seItem.account + "?",operation);
    }
};
...
MgrUser.freezeAccount = function () {
    if (this.check()) {
        ...
        var ajax = new $ax(SecondHandCarAdmin.ctxPath + "/mgr/freeze", function (data) {
            ...
            SecondHandCarAdmin.error("冻结失败!" + data.responseJSON.message + "!");
        });
        ajax.set("userId", userId);
        ajax.start();
    }
...
MgrUser.unfreeze = function () {
    if (this.check()) {
        ...
        var ajax = new $ax(SecondHandCarAdmin.ctxPath + "/mgr/unfreeze", function (data) {
            ...
            SecondHandCarAdmin.error("解除冻结失败!");
        });
        ajax.set("userId", userId);
        ajax.start();
    }
