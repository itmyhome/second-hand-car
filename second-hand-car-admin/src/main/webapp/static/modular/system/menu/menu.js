...
Menu.openAddMenu = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: SecondHandCarAdmin.ctxPath + '/menu/menu_add'
    });
...
Menu.openChangeMenu = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: SecondHandCarAdmin.ctxPath + '/menu/menu_edit/' + this.seItem.id
        });
        this.layerIndex = index;
    }
};
...
Menu.delMenu = function () {
    if (this.check()) {
        ...
        var operation = function(){
            var ajax = new $ax(SecondHandCarAdmin.ctxPath + "/menu/remove", function (data) {
        ...
        SecondHandCarAdmin.confirm("是否刪除该菜单?", operation);
    }
};
