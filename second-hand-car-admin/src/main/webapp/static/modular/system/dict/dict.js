...
Dict.openAddDict = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: SecondHandCarAdmin.ctxPath + '/dict/dict_add'
    });
...
Dict.openDictDetail = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: SecondHandCarAdmin.ctxPath + '/dict/dict_edit/' + Dict.seItem.id
        });
        this.layerIndex = index;
    }
};
...
Dict.delete = function () {
    if (this.check()) {
        ...
        var operation = function(){
            var ajax = new $ax(SecondHandCarAdmin.ctxPath + "/dict/delete", function (data) {
        ...
        SecondHandCarAdmin.confirm("是否刪除字典 " + Dict.seItem.name + "?", operation);
    }
};
